          ...
          <div className="flex items-center">
            <div className="flex-shrink-0">
              <div className="flex items-center">
                ...
                <span className="text-xl font-bold text-gray-900">Auth</span>
              </div>
            </div>
          </div>
...
const Login = () => {
  ...
  // Demonstrate that constants are loaded
  useEffect(() => {
    if (Constants) {
      setConstantsLoaded(true);
      console.log("Constants loaded from common MFE:", Constants);
    }
  }, []);
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    // Validate email using constants
    if (!Constants.VALIDATION_RULES.EMAIL.test(formData.email)) {
    ...
    try {
      // Mock API call using constants from common MFE
      const response = await fetch(Constants.API_ENDPOINTS.AUTH.LOGIN, {
        method: "POST",
        headers: {
      ...
      if (response.status === Constants.HTTP_STATUS.OK) {
        ...
        setError(Constants.ERROR_MESSAGES.VALIDATION_ERROR);
      }
    } catch (err) {
      setError(Constants.ERROR_MESSAGES.NETWORK_ERROR);
    } finally {
      setLoading(false);
    }
  };
          ...
          <div className="text-center mb-8">
            ...
            {constantsLoaded && (
              <div className="mt-2 text-xs text-green-600 bg-green-50 px-2 py-1 rounded">
                ✓ Constants loaded from Common MFE
              </div>
            )}
            {error && (
  ...
  return (
    <div className="min-h-screen bg-gray-50">
      ...
      <Routes>
        ...
        <Route path="/constants-test" element={<ConstantsTest />} />
